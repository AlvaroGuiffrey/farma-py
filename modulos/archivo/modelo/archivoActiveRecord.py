...
from modulos.archivo.modelo.archivoVO import ArchivoVO


class ArchivoActiveRecord(ArchivoVO):
    '''
    Clase que implementa el patrón Active Record en la tabla archivos.

    Permite realizar operaciones del tipo CRUD sobre la tabla archivos de
    ...
    def find(self):
        ...
        if self.cantidad == 1:
            ...
            self.set_estado(dato[2])
            self.set_id_usuario_act(dato[3])
            self.set_fecha_act(dato[4])
            return dato
        else:
            return None
    ...
    def insert(self):
        ...
        @param ArchivoVO: con los datos a insertar en el nuevo registro. 
        """
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        query = ("INSERT INTO archivos (nombre, estado, id_usuario_act, fecha_act)"
                 " VALUES (%s, %s, %s,%s)")
        valor = (self.get_nombre(), self.get_estado(),
