...
from modulos.afip.modelo.afipDocumentoVO import AfipDocumentoVO


class AfipDocumentoActiveRecord(AfipDocumentoVO):
    '''
    Clase que implementa el patrón Active Record en la tabla.
    
    Permite realizar operaciones del tipo CRUD sobre la tabla afip_documentos
    de la base de datos farma en mysql. Hereda los atributos y métodos de la 
    clase AfipDocumentoVO. 
    ...
    def find(self):
        ...
        @return: AfipDocumentoVO con los datos del renglón.
        @return: por el método get_cantidad() se obtiene la cantidad de filas
                afectadas.
        """ 
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        consulta = ("SELECT * FROM afip_documentos WHERE codigo = %s")
    ...
    def find_all(self):
        ...
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        consulta = ("SELECT * FROM afip_documentos")
