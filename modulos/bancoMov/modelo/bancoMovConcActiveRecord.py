...
from modulos.bancoMov.modelo.bancoMovConcVO import BancoMovConcVO


class BancoMovConcActiveRecord(BancoMovConcVO):
    '''
    Clase que implementa el patrón Active Record en la tabla banco_mov_conc.

    Permite realizar operaciones del tipo CRUD sobre la tabla banco_mov_conc de
    la base de datos farma-py en mysql; hereda los atributos y métodos de la
    clase BancoMovConcVO; y La conexión a la base de datos se realiza por una
    ...
    def find(self):
        ...
        @param id: índice de la tabla desde bancoMovConcVO.
        @return: bancoMovConcVO con los datos del renglón.
        @return: por el método get_cantidad() se obtiene la cantidad de filas
                afectadas.
        """
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        consulta = ("SELECT * FROM banco_mov_conc WHERE id = %s")
    ...
    def insert(self):
        ...
        @param bancoMovConcVO: con los datos a insertar en el nuevo registro.
        """
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        valor = (self.get_concepto(), self.get_comentario(),
                self.get_id_grupo(), self.get_estado(),
                self.get_id_usuario_act(), self.get_fecha_act())
        query = ("INSERT INTO banco_mov_conc (concepto, comentario, "
                "id_grupo, estado, id_usuario_act, fecha_act)"
                " VALUES (%s, %s, %s, %s, %s, %s)")
    ...
    def update(self):
        ...
        @param bancoMovConcVO: datos varios.
        """
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        query = ("UPDATE banco_mov_conc SET concepto=%s, comentario=%s, "
                "id_grupo=%s, estado=%s, id_usuario_act=%s, "
    ...
    def count(self):
        ...
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        consulta = ("SELECT COUNT(*) FROM banco_mov_conc")
