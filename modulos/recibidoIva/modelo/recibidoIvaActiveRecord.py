...
from modulos.recibidoIva.modelo.recibidoIvaVO import RecibidoIvaVO


class RecibidoIvaActiveRecord(RecibidoIvaVO):
    '''
    ...
    def find(self):
        ...
        cursor = ccnx.cursor()
        consulta = ("SELECT * FROM recibidos_iva WHERE id = %s")
        ...
        if self.cantidad == 1:
            self.set_id(dato[0])
            self.set_id_recibido(dato[1])
            self.set_condicion(dato[2])
            self.set_neto(dato[3])
            self.set_iva(dato[4])
    ...
    def insert(self):
        ...
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        query = ("INSERT INTO recibidos_iva (id_recibido, condicion, neto,"
                "iva, estado, id_usuario_act, fecha_act)"
                " VALUES (%s, %s, %s, %s, %s, %s, %s)")
        valor = (self.get_id_recibido(), self.get_condicion(), 
                self.get_neto(), self.get_iva(), self.get_estado(),  
    ...
    def count(self):
        ...
        ccnx = ConexionMySQL().conectar()
        cursor = ccnx.cursor()
        consulta = ("SELECT COUNT(*) FROM recibidos_iva")
