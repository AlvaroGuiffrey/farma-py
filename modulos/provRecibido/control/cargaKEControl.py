...
class CargaKEControl():
    ...
    def __init__(self):
        ...
        self.proveedor = 'DROG. KELLERHOFF SA'
        self.nro_doc = 30539756490
        self.archivo = './archivos/KEMovimientos.csv'
        self.form = ''
        self.datos_pg = {}
        self.alertas = []
        self.opciones = []
    ...
    def carga_datos(self, dato):
        ...
        self.prov_recibido.set_fecha(self.fecha_db(dato[0]))
        if dato[1] == "FAC": tipo = 1
        elif dato[1] == "NDE": tipo = 2    
        elif dato[1] == "NCR": tipo = 3
        else: tipo = 999
        self.prov_recibido.set_tipo(tipo)
        self.prov_recibido.set_punto_venta(dato[2][1:5])
        self.prov_recibido.set_numero(dato[2][5:])
        nro_doc = self.nro_doc
        self.prov_recibido.set_nro_doc_emisor(nro_doc)
        self.prov_recibido.set_importe_gravado(self.importe_db(dato[4]))
        self.prov_recibido.set_importe_exento(self.importe_db(dato[3]))
        self.prov_recibido.set_importe_mono(self.importe_db('0'))
        self.prov_recibido.set_importe_no_grav(self.importe_db('0'))
        self.prov_recibido.set_perc_gan(self.importe_db('0'))
        self.prov_recibido.set_perc_iva(self.importe_db('0'))
        self.prov_recibido.set_perc_dgr(self.importe_db(dato[7]))
        self.prov_recibido.set_perc_mun(self.importe_db('0'))
        self.prov_recibido.set_impuesto_int(self.importe_db('0'))
        self.prov_recibido.set_iva(self.importe_db(dato[5]))
        self.prov_recibido.set_importe_total(self.importe_db(dato[9]))
    ...
    def fecha_db(self, fecha_csv):
        ...
        """
        fecha_hora = fecha_csv.split(" ")
        fecha_sola = fecha_hora[0]
        fecha = fecha_sola.split("/")
    ...
    def datos_csv(self,dato):
        ...
        datos_csv = str(tipo)+str(compro)+str(nro_doc)
        return datos_csv
